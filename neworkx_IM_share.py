...
def Yield_IM(queue_order_NSEW):
    ...
    table = PrettyTable()
    table.field_names = ["Vehicle ID", "Origin", "Current Node", "Yield Node for other vehicles", "Dest"]
    ...
    for queue in all_queues:
        for vehicle in queue:
            if vehicle.ID.startswith('emergency'):
                ...
                table.add_row([vehicle.ID, vehicle.origin, vehicle.current_node, adjacent_queue_id, vehicle.destination])
...
#             print(f"\nEV MOVE: Moved Vehicle ID {vehicle.ID} from adjacent queue back to {get_queue_name(original_queue)}")
#         else:
...
# for queue in all_queues:
#     for vehicle in queue:
#         unique_current_nodes.setdefault(vehicle.ID, set())
#         #print("###########", unique_current_nodes)
#     save_thread = threading.Thread(target=save_vehicle_data, args=(queue, ))
#     save_thread.start()
#     #all_thread.append(save_thread)
...
#     # Stop the populating thread gracefully
#     populating_thread.join()

#     # Stop all the save_vehicle_data threads gracefully
#     for save_thread in all_thread:
#         save_thread.join()
